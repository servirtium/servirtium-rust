...
use crate::servirtium_server::{RequestData, ResponseData};
use error::Error;
use fs::File;
...
pub fn check_markdown_data_unchanged<P: AsRef<Path>>(
    ...
    Ok(
        markdown_data.request_body.trim() == request_data.body.trim()
            ...
            && headers_equal(&markdown_data.request_headers, &request_data.headers)
            && headers_equal(&markdown_data.response_headers, &response_data.headers),
    )
}
